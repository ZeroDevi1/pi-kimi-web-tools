...
将 Kimi CLI 官方的 search_web / fetch_url 工具桥接到 Pi Coding Agent。

## 原理

Kimi CLI 是开源项目（https://github.com/MoonshotAI/kimi-cli），其内置的 `search_web` 和 `fetch_url` 工具通过独立的 HTTP API 端点实现：
...
| 工具 | 功能 | 参数 |
|------|------|------|
| `search_web` | 网页搜索 | `query`, `limit` (1-20), `include_content` (bool) |
| `fetch_url` | 网页抓取 | `url` |
...
### `/search` vs `search_web` 工具的区别

- **`search_web`（工具）**：模型自主判断何时搜索，用户无感知
...
- `search_web` 的 `include_content=true` 会消耗大量 token，limit 较大时避免开启
