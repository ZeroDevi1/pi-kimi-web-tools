	...
	try {
		...
		if (!response.ok) {
			...
			if (response.status >= 500) {
				console.warn(
					`[pi-kimi-web-tools] Kimi fetch service returned ${response.status} for ${url}, falling back to local HTTP GET`,
				);
		...
		if (e.name === "AbortError") {
			...
			console.warn(
				`[pi-kimi-web-tools] Kimi fetch timeout for ${url}, falling back to local HTTP GET`,
			);
			return await fetchWithHttpGet(url);
		}
		throw e;
	...
	pi.registerCommand("fetch", {
		...
		handler: async (args, ctx) => {
			...
			try {
				...
				pi.sendUserMessage(`Fetched content from ${targetUrl}:\n\n${preview}`, {
					deliverAs: "followUp",
				});
			} catch (e: any) {
